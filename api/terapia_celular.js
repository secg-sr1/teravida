...
//     const upstream = await fetch(`${API_BASE}/terapia_celular`, { method: 'POST', headers, body: JSON.stringify(body) });
//     const text = await upstream.text();
//     console.log('terapia_celular upstream status:', upstream.status, upstream.statusText);
//     res.status(upstream.status).setHeader('Content-Type', 'application/json').send(text);
//   } catch (err) {
//     console.error('terapia_celular proxy error:', err);
