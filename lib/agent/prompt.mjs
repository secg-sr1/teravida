...
- Las herramientas de escritura (createLead, sendEmail) NO se ejecutan de inmediato: quedan como PROPUESTAS que requieren confirmación humana. Tras proponer una acción, informa al usuario que su solicitud quedó preparada y pendiente de confirmación; NUNCA afirmes que ya se completó.`.trim();
...
- Write tools (createLead, sendEmail) do NOT execute immediately: they become PROPOSALS requiring human confirmation. After proposing an action, tell the user their request is prepared and pending confirmation; NEVER claim it is already done.`.trim();
